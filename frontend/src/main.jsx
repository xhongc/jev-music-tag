...
const LABELS = { title: '标题', artist: '艺术家', album: '专辑', albumartist: '专辑艺术家', genre: '流派', year: '年份', language: '语言', tracknumber: '音轨', duration: '时长', bitrate: '码率', sample_rate: '采样率' }
const ORDER = ['title', 'artist', 'album', 'albumartist', 'genre', 'year', 'language', 'tracknumber', 'duration', 'bitrate', 'sample_rate']
...
function App() {
  ...
  const visibleFields = ORDER.filter(key => metadata && metadata[key] !== undefined)
  ...
  return <main className="shell">
    ...
    <section className="content-grid"><div className="metadata-section"><div className="section-title"><div><span className="step">2</span><h2>音乐信息</h2></div>{metadata && <span className="muted">{metadata.filename || '当前文件'}</span>}</div>{metadata ? <div className="metadata-card">{visibleFields.map(key => <div className={`metadata-item ${suggestions[key] ? 'suggested' : ''}`} key={key}><span>{LABELS[key] || key}</span><strong>{formatValue(key, suggestions[key] || metadata[key])}</strong>{suggestions[key] && <small>Jev 建议</small>}</div>)}</div> : <div className="empty-card"><div className="empty-icon">♫</div><strong>还没有读取音乐</strong><p>输入文件位置后，这里会显示清晰的音乐信息。</p></div>}</div>
